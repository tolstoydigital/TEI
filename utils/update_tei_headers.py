...
def process_comments(row: dict, data: dict):
...
def process_diaries(row: dict, data: dict):
    # date_H = ''
    first_date = row['first_date']
    last_date = row['last_date']
    # print(row['first_date'], row['last_date'])
    ...
    # print(first_date, last_date, row)
    if ':' in first_date:
        not_before, not_after = first_date.split(':')
        date_H = f'<date notBefore="{not_before}" notAfter="{not_after}"/>'
    # print(row)
    # print(row['Техническая колонка'])
    data['date_H'] = date_H
    # print(date_H)


def process_krug(row: dict, data: dict):
    months = {
        '1': 'января',
        '2': 'февраля',
        '3': 'марта',
...
def process_letters(row: dict, data: dict):
    ...
    if not isinstance(row["Адресат - берем из xml  (берем из колонки H из адресатов)"], str):
        ...
        # print(row["Адресат - берем из xml  (берем из колонки H из адресатов)"], str)
...
def process_notes(row: dict, data: dict):
    # date_H
    if row['Техническая колонка'] == '1':
...
def process_works(row: dict, data: dict):
    # language
    language = row['language']
    data['language_K'] = 'ru' if language is None else ut.convert_language_to_iso(language)
    # publ
    # data['published_AY'] = 'publ' if row['Published TEI'] == 'publ' else 'not_publ'
    # data['published_AY'] = row['(not) published SIMPLE ']
...
FOLDERS_DATA = {
    'azbuka': {
        ...
        'column_names': {
            ...
            'title_level_s_F': 'book',  # there are some without
...
def fix_date_tag(root: etree._Element):
    ...
    for tag in date_tags:
        ...
        if single_date_match is not None:
            ...
            tag.text = None
            # print(single_date_match.group(1), etree.tostring(tag, encoding='unicode'))
...
def remove_empty_catref(root: etree._Element):
...
def remove_header_tags_with_none_text(root: etree._Element):
    """If tag has text 'None' (not `is None`!), remove the tag."""
    # title_tags = root.xpath('//ns:title', namespaces={'ns': XMLNS})
    header_tag = root.xpath('//ns:teiHeader', namespaces={'ns': XMLNS})[0]
...
def indent_header_tag(root: etree._Element):
    header_tag = root.xpath('//ns:teiHeader', namespaces={'ns': XMLNS})[0]
    etree.indent(header_tag)


def restore_p_uuid_in_header(root, path_to_folder, filename):
...
def update_folder(path_to_folder: Path, path_to_sheet: Path, path_to_header_template: Path) -> None:
    ...
    for filename in os.listdir(path_to_folder):
        ...
        check_root = etree.fromstring(result_text.encode())
