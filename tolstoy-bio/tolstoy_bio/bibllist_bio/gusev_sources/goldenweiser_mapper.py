...
class GoldenweiserMapper:
    ...
    def map(self) -> None:
        ...
        for segment, documents in tqdm(list(gusev_documents_by_bibl_segment.items())):
            for document in documents:
                ...
                gusev_id_to_sources[document.get_id()].update(
                    matching_sources
                )
        ...
        IoUtils.save_as_json(
            ...
            os.path.join(os.path.dirname(__file__), "gusev-to-goldenweiser.json"),
            indent=2,
        )
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__), "data/parsed_bibl_segments/goldenweiser-segments-to-dates.json"
            )
        )
