...
class TolstayaDiariesMapper:
        ...
        IoUtils.save_as_json(
            {
                ...
                for gusev_id, sources in gusev_id_to_tolstaya_diary_sources.items()
            },
            os.path.join(os.path.dirname(__file__), "gusev-to-tolstaya-diaries.json"),
            indent=2,
        )
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__), "segments-to-dates-tolstaya-diaries.json"
            )
        )
