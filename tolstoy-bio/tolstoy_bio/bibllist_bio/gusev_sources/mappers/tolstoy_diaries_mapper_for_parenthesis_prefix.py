...
from ..document.tolstoy_diary_tei_document import TolstoyDiaryTeiDocument
...
class TolstoyDiariesMapperForParenthesisPrefix:
    ...
    def map_to_sources(self) -> dict[str, set[Source]]:
        ...
        source_date_to_source_filenames: dict[str, set[str]] = defaultdict(set)

        for source_path, source_filename in tqdm(
            list(zip(potential_source_paths, potential_source_filenames)),
            "[Tolstoy's diaries with starting parenthesis] Hashing sources by date",
        ):
            source_document = TolstoyDiaryTeiDocument(source_path)
            source_dates = source_document.get_creation_dates()

            for source_date in source_dates:
                source_date_to_source_filenames[source_date].add(source_filename)

        gusev_id_to_sources: dict[str, set[Source]] = defaultdict(set)

        for segment, documents in tqdm(
            list(gusev_documents_by_bibl_segment.items()),
            "[Tolstoy's diaries with starting parenthesis] Mapping to sources",
        ):
                ...
                for date in dates:
                    matching_sources.update(
                        [
                            ...
                            for name in source_date_to_source_filenames[date]
                        ]
                    )
    ...
    def map_to_source_ids(self) -> dict[str, list[str]]:
        gusev_id_to_sources = self.map_to_sources_and_save()
    ...
    def map_to_sources_and_save(self) -> dict[str, set[Source]]:
        ...
        IoUtils.save_as_json(
            ...
            os.path.join(
                os.path.dirname(__file__),
                "../data/linking_results/gusev-to-tolstoy-diaries-with-parenthesis-prefix.json",
            ),
            indent=2,
        )
    ...
    def _get_gusev_documents_by_bibl_segment(
        ...
        for document in tqdm(
            gusev_documents,
            "[Tolstoy's diaries with starting parenthesis] Hashing Gusev's documents by <bibl> segment",
        ):
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__),
                "../data/parsed_bibl_segments/tolstoy-diary-parenthesis-prefix-segments-to-dates.json",
            )
        )
