...
class TolstayaJournalsMapper:
    ...
    def map_to_sources(self) -> dict[str, set[Source]]:
        ...
        tolstaya_journal_filenames = [
            filename
            for filename in IoUtils.get_folder_contents_names(
                os.path.join(
                    os.path.dirname(__file__),
                    "../../../tolstaya_diaries/data/xml/by_entry",
                )
            )
            if "tolstaya-s-a-journals" in filename
        ]

        gusev_id_to_tolstaya_journal_sources: dict[str, str] = defaultdict(set)
        ...
        for segment, documents in tqdm(list(gusev_documents_by_bibl_segment.items())):
            for document in documents:
                ...
                if possibly_dates is None:
                    ...
                    assert metadata.start_date.year == metadata.end_date.year
                ...
                matching_tolstaya_journal_sources: set[Source] = set()

                for date in dates:
                    matching_tolstaya_journal_sources.update(
                        [
                            Source(
                                linkage_criterion=linkage_criterion,
                                document_id=name.replace(".xml", ""),
                            )
                            for name in tolstaya_journal_filenames
                            if date in name
                        ]
                    )

                gusev_id_to_tolstaya_journal_sources[document.get_id()].update(
                    matching_tolstaya_journal_sources
                )

        return gusev_id_to_tolstaya_journal_sources
    ...
    def map_to_sources_and_save(self) -> None:
        ...
        IoUtils.save_as_json(
            ...
            os.path.join(os.path.dirname(__file__), "../data/linking_results/gusev-to-tolstaya-journals.json"),
            indent=2,
        )
    ...
    def _get_gusev_documents_by_bibl_segment(
        ...
        for document in tqdm(gusev_documents, "[Tolstaya's journals] Hashing Gusev's documents by <bibl> segment"):
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__), "../data/parsed_bibl_segments/tolstaya-journal-segments-to-dates.json"
            )
        )
