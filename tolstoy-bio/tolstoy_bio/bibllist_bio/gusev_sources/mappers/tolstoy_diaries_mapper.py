...
class TolstoyDiariesMapper:
    ...
    def map_to_sources(self) -> dict[str, set[Source]]:
        ...
        for source_path, source_filename in tqdm(
            ...
            "[Tolstoy's diaries] Hashing sources by date",
        ):
        ...
        for segment, documents in tqdm(list(gusev_documents_by_bibl_segment.items()), "[Tolstoy's diaries] Mapping to sources"):
    ...
    def map_to_sources_and_save(self) -> None:
        ...
        IoUtils.save_as_json(
            ...
            os.path.join(
                os.path.dirname(__file__),
                "../data/linking_results/gusev-to-tolstoy-diaries.json",
            ),
            indent=2,
        )
    ...
    def _get_gusev_documents_by_bibl_segment(
        ...
        for document in tqdm(
            gusev_documents,
            "[Tolstoy's diaries] Hashing Gusev's documents by <bibl> segment",
        ):
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__),
                "../data/parsed_bibl_segments/tolstoy-diary-segments-to-dates.json",
            )
        )
