...
class TolstayaJournalsMapper:
        ...
        IoUtils.save_as_json(
            {
                ...
                for gusev_id, sources in gusev_id_to_tolstaya_journal_sources.items()
            },
            os.path.join(os.path.dirname(__file__), "gusev-to-tolstaya-journals.json"),
            indent=2,
        )
    ...
    def _get_bibl_segment_to_possible_dates_table(self) -> dict[str, list[str] | None]:
        segments_by_type_as_json = IoUtils.read_as_text(
            os.path.join(
                os.path.dirname(__file__), "segments-to-dates-tolstaya-journals.json"
            )
        )
