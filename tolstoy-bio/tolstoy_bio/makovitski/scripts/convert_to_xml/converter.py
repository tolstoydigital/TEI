...
class FebHtmlToTeiXmlConverter:
    ...
    def remove_scripts(self):
        ...
        invisible_script_block = self.tei_soup.find('div', style="display:none;")

        if not invisible_script_block:
            raise RuntimeError('Failed to find invisible footer script block')

        invisible_script_block.decompose()
