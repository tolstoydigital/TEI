...
class TolstayaLetterFebHtmlToTeiXmlConverter:
    """
    ...
    def convert_to_tei(self):
        ...
        transformations = [
            ...
            self.transform_images, # check for classes
            self.transform_footnotes, # check classes
            self.transform_notes, # check later
            self.transform_styles,
            # self.divide_into_letters, # not needed for tolstaya
    ...
    def populate_tei_header(self):
        ...
        if self.source_url == "http://feb-web.ru/feb/tolstoy/critics/tpt/tpt2312-.htm?cmd=p":
            ...
            date_element = f'<date from="{iso_start_date}" to="{iso_end_date}" />' \
                if iso_end_date else f'<date when="{iso_start_date}" />'
            
        assert date_element is not None, "Failed to parse date element"
        ...
        tei_header = bs4.BeautifulSoup(f'''
            <teiHeader>
                ...
                <profileDesc>
                    ...
                    <textClass>
                        <catRef ana="#letters" target="type" />
                    </textClass>
                </profileDesc>  
            </teiHeader>''', 'xml')
    ...
    def transform_footnotes(self):
        ...
        if self.tei_soup.select_one('div.footnotesp'):
            self.transform_div_footnotes(starting_footnote_index=last_footnote_index + 1)
